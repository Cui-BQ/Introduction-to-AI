...
def color_histogram(image,method):
	...
	# Helper function to calculate the gray-tone of a pixel 
	# by passing in the R, G, B value of this pixel.
	...
	if method == 'gray_8':
		...
		# For each pixel in image, find luminance. 
		for x in range(image.shape[1]):
			for y in range(image.shape[0]):
				...
				# Find which bin to put this pixle, and put to this bin.
				bin = luminance // 32
				hist_vector[int(bin)] += 1
		...
		for x in range(image.shape[1]):
			for y in range(image.shape[0]):
				...
				# put to this bin.
				hist_vector[int(np.round(luminance))] += 1

	elif method == 'rgb':
		num_bins = 256 # change this to 8 or 256 according to conclusion above
		hist_vector = np.zeros((num_bins*3,))

		# rgb histogram has 3 channels, hist_vector[0-255] for R, [256-511] for G, [512-767] for B.
		# For each pixel in image, distribute its RGB value to the corresponding bin.
		for x in range(image.shape[1]):
			for y in range(image.shape[0]):
				R = image[y, x, 0]
				G = image[y, x, 1]
				...
				hist_vector[int(R)] += 1
				hist_vector[int(G) + num_bins] += 1
				hist_vector[int(B) + num_bins*2] += 1
...
def lbp_histogram(image,method):
	...
	# Helper function to compute the 3*3 lbp of the gray_image on (x, y).
	def compute_lbp(x, y, gray_image) -> int:
		current = gray_image[y, x]
		neighbor = [0]*8
		neighbor[0] = 1 if gray_image[y-1, x-1] > current else 0
		neighbor[1] = 1 if gray_image[y-1, x] > current else 0
		neighbor[2] = 1 if gray_image[y-1, x+1] > current else 0
		neighbor[3] = 1 if gray_image[y, x+1] > current else 0
		neighbor[4] = 1 if gray_image[y+1, x+1] > current else 0
		neighbor[5] = 1 if gray_image[y+1, x] > current else 0
		neighbor[6] = 1 if gray_image[y+1, x-1] > current else 0
		neighbor[7] = 1 if gray_image[y, x-1] > current else 0
		return int("".join(str(x) for x in neighbor), 2)
	...
	if method == 'whole_image':
		...
		# For each pixel, ignore the boundary pixels, compute Local Binary Pattern.
		for x in range(1, image.shape[1]-1):
			for y in range(1, image.shape[0]-1):
				current_lbp = compute_lbp(x, y, gray_image)
				# put the current_lbp of this pixel to its bin.
				hist_vector[current_lbp] += 1


	elif method == 'grid_image':
		# you can remove and rewrite the following 2 lines according to your implementation
		num_grids = (image.shape[1]//16) * (image.shape[0]//16)
		hist_vector = np.zeros((32*num_grids,))

		# For each grid, compute a 32-bin histogram
		for gx in range(image.shape[1]//16):
			for gy in range(image.shape[0]//16):
				for x in range(1, 15):
					for y in range(1, 15):
						current_lbp = compute_lbp(x+gx*16, y+gy*16, gray_image)
						bin = int(current_lbp//8)
						# put the current_lbp of this pixel to its bin.
						hist_vector[bin + gx + gy*image.shape[1]//16] += 1
